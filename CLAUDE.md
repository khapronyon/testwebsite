## Stack
- React 18 + Vite 6 + Tailwind CSS v3
- GitHub per il versioning
- Netlify per il deploy (build: `npm run build`, publish: `dist`)

## Struttura del progetto
src/components/   → componenti riutilizzabili (PascalCase)
src/pages/        → pagine principali
src/assets/       → immagini, font, icone
src/styles/       → CSS globale e variabili
public/           → file statici

## Flusso di lavoro
- Prima di ogni task complesso, esponi il piano in massimo 5 righe e attendi approvazione
- Produci sempre file completi e pronti all'uso, mai troncati
- Se un task tocca più file, completali tutti prima di fermarti
- Se incontri un ostacolo, proponimelo senza procedere autonomamente
- Nessuna spiegazione del codice salvo richiesta esplicita
- Styling con Tailwind; CSS custom solo se strettamente necessario
- Un file per componente
- Non eseguire commit né push: li gestisce l'utente manualmente

## Branch
- `main` → produzione
- `feature/nome` → sviluppo

## Focus attuale
- [aggiorna questa sezione a ogni nuova fase]
